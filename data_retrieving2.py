...
import os
import pickle
import Homemade_Functions as hom
import requests
...
def data_retrieving(username,path0,auth_header,loc2name,parameter2,path2,folder_name,
                    subset,name,experiments_parameters,digits):
    """
    ...
    a='capture_datetime_utc'
    T='air_temperature_celsius'
    L='light' #  mol m-² d-1
    W='soil_moisture_percent'
    F='fertilizer_level'
    ...
    if hom.Yes_or_No(delta_down_limit) == True:
        ...
        data_treshold = pan.DataFrame([[15,treshold_delta_down_15min],[1455,treshold_delta_down_1day]],
                                      columns = ['minutes','percentage'])
    ...
    if append_data == False: 
        ...
        last_synchronisation_parameters.rename(columns = {0:'Sensor',
                      1:'first time (raw data)',2:'last time (raw_data)',3:'Time for the last True data',
                      4:'Imputed values before the first real value',5:'number of imputed data or NA',
                      6:'data_following_value_real_time',7:'data_following_value_temperature',
                      8:'data_following_value_light', 9:'data_following_value_water_content', 
                      10:'data_following_value_fertilizer', 11:'data_following_value_notifications'}, 
                      inplace =True)
 ...
 # # # # # # # #
    ...
    for Sensor in subset:
        ...
        if append_data == True: 
           Start = last_synchronisation_parameters.loc[Sensor]['Time for the last True data'] + \
...
#                            'to_datetime_utc': ED})
                        ...
                        req = requests.get('https://api-flower-power-pot.parrot.com/sensor_data/v6' +
                            '/sample/location/{loc_id}'.format(loc_id = loc_id),
                            headers = auth_header,
                            params={'from_datetime_utc': SD,
                            'to_datetime_utc': ED})
                        ...
                        print(attempt,'attempt(s):',
                          'Failed to establish a connection with the website. waiting time of two minutes before the next attempt')
                        attempt += 1
...
# light in mol/m2/d
            ...
            d1_ = [{'capture_datetime_utc' : 'NaN',
                    'air_temperature_celsius' :'NaN', 
                    'light' : 'NaN',
                    'soil_moisture_percent' : 'NaN', 
                    'fertilizer_level' : 'NaN'}]
                    
            raw_data = pan.DataFrame.from_dict(d1_,orient = 'columns',dtype = None)
            print(' No data at all for this sensor')
            
        else:
            raw_data = pan.DataFrame.from_dict(d1,orient = 'columns',dtype = None)
            digits_raw_data = pan.Series(digits, index = ['air_temperature_celsius',
                                                          'light',
...
###----------------###----------------###----------------###----------------###----------------###----------------###----------------###
            ...
            if Y1 <= 7 :                #(a)
                ...
                d2[0] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                                 hom.date_format(theoretical_time),a0,T0,L0,W0,F0,"True","True"]
                ...
                if append_data == True:
                    ...
                    if last_synchronisation_parameters.loc[Sensor]['data_following_value_notifications'] == 'True':
                        d2[0] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),\
                                hom.date_format(theoretical_time),\
                                last_synchronisation_parameters.loc[Sensor]['data_following_value_real_time'],\
                                last_synchronisation_parameters.loc[Sensor]['data_following_value_temperature'],\
                                last_synchronisation_parameters.loc[Sensor]['data_following_value_light']+0.1,\
                                last_synchronisation_parameters.loc[Sensor]['data_following_value_water_content'],\
                                last_synchronisation_parameters.loc[Sensor]['data_following_value_fertilizer'],\
                                "True","True"]
                            # there is a +0.1. see the end of the script.
                        theoretical_time += delta
                        d2[1] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                                       hom.date_format(theoretical_time),a0,T0,L0,W0,F0,"True","True"]
                        z = 1
                    else:
                        d2[0] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                                    hom.date_format(theoretical_time),str(theoretical_time),T0,L0,W0,F0,"Pasting","True"]
                        theoretical_time += delta
                        d2[1] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                                    hom.date_format(theoretical_time),a0,T0,L0,W0,F0,"True","True"]
                        z = 1
                ...
                for z in range(0,int(nb_NA_required)-1):#-1 in order to paste the values from the next available time.
                    d2[z] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                         hom.date_format(theoretical_time),str(theoretical_time),'NaN','NaN','NaN','NaN',"imputed","True"]
                    theoretical_time += delta
                    z += 1
...
###----------------###----------------###----------------###----------------###----------------###----------------###----------------###
                ...
                if y == "Skip":
                    #remove the i
                    print("the values for",i,"was deleted",a_i,"%.1f" %T_i,"%.2f" %L_i,"%.4f" %W_i, "%.2f" %F_i) 
                    theoretical_time -= delta
                    continue
                
                elif y != "Skip" and y != "NaN": # the real time is close enough to the theoretical time
                    d2[z] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                                hom.date_format(theoretical_time),a_i,T_i,L_i,W_i,F_i,"True","True"]
...
###----------------###----------------###----------------###----------------###----------------###----------------###----------------###
                    ...
                    if nb_NA <= 16:                              #(a)
                        for k in range(1,nb_NA+1):
                            ...
                            Fx = hom.imputation(k,0,F_i_1,nb_NA+1,F_i)
                            d2[z] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                                        hom.date_format(theoretical_time),str(theoretical_time),Tx,Lx,Wx,Fx,"imputed","True"]
                            theoretical_time += delta
                            z += 1
                        d2[z] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                                        hom.date_format(theoretical_time),a_i,T_i,L_i,W_i,F_i,"True","True"]
                        ...
                        for k in range(1,nb_NA+1):
                            ...
                            theoretical_time += delta
                            z += 1
                        d2[z] = [str(theoretical_time),Sensor,
                                   hom.Time(theoretical_time),hom.date_format(theoretical_time),a_i,T_i,L_i,W_i,F_i,"True","True"]
...
##-----------##-----------##-----------##-----------##-----------##-----------##-----------##-----------##----------- 
                ...
                if Na == True:
                    for element in range(len(first_date_with_NaN)):
                        missing_data [element] = {
                        'sensor': Sensor,
                        'first_date_with_NaN':first_date_with_NaN[element],
                        'last_date_with_NaN':last_date_with_NaN[element]}
            ...
            while theoretical_time < End:
                ...
                theoretical_time += delta
                d2[z] = [str(theoretical_time),Sensor,hom.Time(theoretical_time),
                        hom.date_format(theoretical_time),np.NaN,np.NaN,np.NaN,np.NaN,np.NaN,'no more data',"True"]
            ...
            x=0
            while theoretical_time <= End:
                d2[x] = [str(theoretical_time),Sensor,np.NaN,np.NaN,np.NaN,np.NaN,np.NaN,np.NaN,np.NaN,'True','True']
                theoretical_time += delta
                x += 1
        ...
        Tp_Light_WC.rename(columns = {0:'corrected_time',
                                      1:'Sensor',
                                      2:"Julian_time",
                                      3:"date",
                                      4:"real_time",
        ...
        if hom.Yes_or_No(delta_down_limit) == True:
            Tp_Light_WC_before_cor = Tp_Light_WC[Tp_Light_WC.notifications != 'True']
            Tp_Light_WC = Tp_Light_WC[Tp_Light_WC.notifications == 'True']
            ...
            Tp_Light_WC_before_cor['water_content'] = Tp_Light_WC_before_cor.water_content.replace(-1,np.NaN)
            
            Tp_Light_WC['water_content'] = Tp_Light_WC.water_content.interpolate(method = 'time')
                          
            dfs = [Tp_Light_WC,Tp_Light_WC_before_cor]
            Tp_Light_WC = pan.concat( dfs,axis=0)
            Tp_Light_WC = Tp_Light_WC.sort_index(axis=0)
    
        digits_Tp_Light_WC = pan.Series(digits, index = ["temperature","light",'water_content','fertilizer'])
        ...
        if stop_writing_data != end:
            ...
            if len(data_following_value) == 1:
                ...
                data_following_value_fertilizer = data_following_value["fertilizer"][0]
                ...
                data_following_value_fertilizer = np.NaN
    ...
    raw_data_all_sensors = raw_data_all_sensors[['Sensor',
    ...
    data2.rename(columns = {0:'Sensor',
                            1:'first time (raw data)',
                            2:'last time (raw_data)',
                            3:'Time for the last True data',
                            4:'Imputed values before the first real value',
                            5:'number of imputed data or NA',
                            6: 'data_following_value_real_time',
                            7: 'data_following_value_temperature',
                            8: 'data_following_value_light',
                            9: 'data_following_value_WC',
                            10:'data_following_value_fertilizer',
                            11:'data_following_value_notifications'},inplace = True)
