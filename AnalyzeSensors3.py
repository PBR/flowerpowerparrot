...
path0 = os.getcwd()
    ...
    # download the input file and parameters
input_file = path0+'\\input_file.xlsx'
...
digits = [int(experiments_parameters.loc['digit_temperature']['Reply']),
                ...
                int(experiments_parameters.loc['digit_fertilizer']['Reply'])]
                                        
Account_list_beginning = ['robert.vanloo@wur.nl',
     'flowerpower.pbr1@gmail.com',
     'flowerpower.pbr2@gmail.com',
     'flowerpowertest1@gmail.com',
     'Jd.ligthart@bejo.nl']
Account_reference = ['first_account', 'second_account','third_account','fourth_account','fifth_account']
...
Account_list = list(Account_list_beginning)

test_while_loop = False

while test_while_loop == False:
    experiment_name_selected = experiments_parameters.loc['experiment_name']['Reply']
    if (experiment_name_selected in experiments_sensors.columns) == True:
         test_while_loop = True
    else:
        continue

details = experiments_details.loc[experiment_name_selected]['details']
Account = experiments_details.loc[experiment_name_selected]['account']
...
# we need to keep in memory the experiment name selected by the user.
experiment_name = experiment_name_selected
...
for username in Account_list:
    ...
    selected_sensors = account_selected [account_selected [experiment_name_selected] != 0]
    ...
    account_number = Account_list_beginning.index(username)+1
    ...
    print('The retrieving is now done')
    #==============================================================================
    ...
    while several_experiment_by_account == True:
        ...
        experiment = standardization_parameters[6]
        ...
        test2 = retrieved_data[2] >= standardization_parameters[2]
        ...
        #### start if condition A
        if str(experiments_details.loc[experiment_name]['standardization']) == 'True' and test == True:
            print("Standardization is running")
            ...
            processed_data = dp.data_processing(path0,path4,path2,experiments_parameters,selected_sensors,experiment_name,folder_name,experiments_details,subset,digits)
