...
#==============================================================================
    ...
    if (os.path.exists(path4)) == True and hom.Yes_or_No(Correction) == True:
        try:
            ...
            Correction_Fertilizer = experiments_parameters.loc['Correction_Fertilizer']['Reply']
        except:
            Correction = 'No'
            print('there is something going wrong with the standardization or with the correction \n'
                'parameters files, to avoid any crash, the Correction parameter has been set to False')
    ...
    number_of_above_crop_sensor = len(above_crop_sensors)
    ...
    print('above_crop_sensors',above_crop_sensors)
    for sensor in sensor_selection:
        ...
        digits_raw_data = pan.Series(digits, index = ["temperature","light",'water_content', 'fertilizer'])
        ...
        if (os.path.exists(path4)) == True and hom.Yes_or_No(Correction) == True:
            ...
            if hom.Yes_or_No(Correction_Fertilizer) == True:  
                try:
                    slope_fertilizer = correction_parameters.loc[sensor]['slope_fertilizer']
                    intercept_fertilizer = correction_parameters.loc[sensor]['Intercept_fertilizer']
                    raw_data['fertilizer'] = (raw_data['fertilizer']*slope_fertilizer) + intercept_fertilizer
                    raw_data['fertilizer'] = raw_data['fertilizer_content'].replace(- intercept_fertilizer,0)
                except:
                    print('fertilizer:',sensor,'not present in the correction parameters file')
    ...
    #####    
        ...
        if sensor in above_crop_sensors:
            if hom.Yes_or_No(control) == True:
                ...
                if number_of_above_crop_sensor-1 == position_in_the_list:
                    ...
                    os.mkdir(path10)
                    above_crop_sensors_15_minutes_data = open(path10+'\\data_above_crop_sensors.txt','a')
...
# check whether next line solves the Figure1 (not responding) window flicking on and off                    
                    plt.close('all')
    ...
    #####   
        ...
        if sensor in above_crop_sensors:
            if hom.Yes_or_No(control) == True:
                ...
                if position_in_the_list == 0:
                    ...
                    daily_data_above_crop_sensors[['temperature_average','light_average','water_average']] = daily_data_above_crop_sensors[['temperature_average','light_average','water_average']].apply(pan.to_numeric)
                ...
                if position_in_the_list > 0:
                    ...
                    daily_data_above_crop_sensor_bis[['temperature_average','light_average','water_average']] = daily_data_above_crop_sensor_bis[['temperature_average','light_average','water_average']].apply(pan.to_numeric)
                ...
                if number_of_above_crop_sensor-1 == position_in_the_list:
                    ...
                    daily_data_above_crop_sensors = hom.correct_running_average_result (daily_data_above_crop_sensors,'run_average_control_light_cum','average_control_light_cum',11)
            
                    # print the result in a new file 
                    path12 = path_daily_data+'\\'+'above_crop_sensors'
                    os.mkdir(path12)
                    above_crop_sensors_daily_data = open(path12+'\\daily_data_above_crop_sensors.txt','a')
            ...
            if number_of_above_crop_sensor > 0 :
                ...
                Processed_data = hom.correct_running_average_result (Processed_data,'run_average_light_average_cum','light_average_cum',11)
        ...
        if hom.Yes_or_No(plot) == True:
            if (sensor in above_crop_sensors) == False:
                ...
                # water
                water = Processed_data.plot(y =['water_average','wc_delta_up','wc_delta_down'],
                                            legend = False,
                                            subplots=True,
                                            figsize=(13,7),
                                            rot = 45,
                ...
                [water[2].set_ylabel('wc_delta_down (%)')]
                plt.subplots_adjust(hspace=.7)
